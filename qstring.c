...
qstring qstring_new_buffer(const char* buffer, size_t n) {
    qstring ret = {.n = 0, .data = malloc(n + 1)};
    if (ret.data == NULL) {
        return ret;
    }
    memcpy(ret.data, src, n);
    ret.data[n] = '\0';
    ret.len = n;
    return ret;
}
...
qstring qliteral_new(const char* cs) {
    qstring ret = {.n = strlen(cs), .data = cs};
    return ret;
}
...
qstring qstring_remove(qstring qs, size_t start, size_t n) {
    ...
    qstring ret = {.n = 0, .data = malloc(qs.len - n)};
    if (ret.data == NULL) {
        return ret;
    }
    memcpy(ret.data, qs.data, start);
    memcpy(ret.data + start, qs.data + start + n, qs.len - n);
    ret.len = qs.len - n;
    return ret;
}

qstring qstring_concat(qstring qs1, qstring qs2) {
    qstring ret = {.n = 0, .data = malloc(qs1.len + qs2.len + 1)};
    if (ret.data == NULL) {
        return ret;
    }
...
qstring qstring_replace_all(qstring qs, qstring before, qstring after) {
    ...
    qstring ret = {.len = 0, .data = malloc(newlen + 1)};
    if (ret.data == NULL) {
        return ret;
    }
    size_t index = 0;
...
size_t qstring_count(qstring qs, qstring datum) {
    ...
    for (size_t i = 0; i < qs.len; i++) {
        if (memcp(qs.data + i, datum.data, datum.len) == 0) {
            count++;
            i += datum.len;
        }
    }
...
bool qstring_startswith(qstring qs, qstring prefix) {
    return qs.len >= prefix.len && memcmp(qs.data, prefix.data, prefix.len) == 0;
}

bool qstring_endswith(qstring qs, qstring endswith) {
    return qs.len >= prefix.len &&
        memcmp(qs.data + (qs.len - prefix.len), prefix.data, prefix.len) == 0;
}
